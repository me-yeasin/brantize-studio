    ...
    async function fetchProject() {
      try {
        ...
        const mappedProject: Post = {
          ...
          completedDate: new Date(project.publishedAt).toLocaleDateString("en-US", {
            year: "numeric",
            month: "long",
          }),
          duration: "3 months", // This is hardcoded as it's not in DB schema
          industry: "Technology", // This is hardcoded as it's not in DB schema
          tags: project.technologies || [],
          challenge: project.description, // Use description as challenge since we don't have a separate field
          solution: project.description, // Use description as solution since we don't have a separate field
          implementation: project.description, // Use description as implementation since we don't have a separate field
          technologies: project.technologies || [],
          team: ["Project Team"], // Hardcoded as it's not in DB schema
          client: project.client,
          features: project.features?.map((feature: string) => ({
            icon: "fas fa-check",  // Default icon as it's not in DB schema
            title: feature,
            description: feature,
          })) || [],
          process: project.process?.map((step: { title: string; description: string; image?: string }) => ({
            icon: "fas fa-arrow-right", // Default icon
            title: step.title,
            description: step.description,
          })) || [],
          gallery: project.gallery || [],
          results: project.results?.map((result: { title: string; value: string; icon?: string }) => ({
          ...
          testimonial: project.testimonial ? {
            ...
            initials: project.testimonial.author ? project.testimonial.author.split(" ").map((n: string) => n[0]).join("") : "AA",
          } : {
            content: "Great work!",
            author: "Client",
            position: "Position",
          ...
          relatedProjects: project.relatedProjects ? project.relatedProjects.map((related: { title: string; excerpt: string; coverImage: string }) => ({
            title: related.title,
